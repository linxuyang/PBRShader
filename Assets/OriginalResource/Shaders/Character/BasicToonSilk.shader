Shader "MC/Character/Toon/BasicToonSilk"
{
    Properties 
    {
        [Header(Basics)]
    ...
    half _SpecPower;
    half3 _AnisoSpecColor;
    half _AnisoSpecularPosition;
    ...
    sampler2D _TangentFlow;

    half3 GetTangentFlow(half2 uv, half3 normal, half3x3 tbn, out half specMask)
    {
        half3 tangentFlowVal = tex2D(_TangentFlow, uv).rgb;
        specMask = tangentFlowVal.b;
        tangentFlowVal.xy = tangentFlowVal.yx * 2 - 1;
        tangentFlowVal.y *= -1;
        half3 tangent = normalize(half3(tangentFlowVal.xy, 0));
        tangent = mul(tangent, tbn);
        tangent -= normal * dot(tangent, normal);
        return tangent;
    }

    half3 SilkSpecular(half3 tangent, half3 halfVector, half3 albedo, half metallic, half smoothness, half nDotL)
    {
        half3 specColor = lerp(kDielectricSpec.rgb, albedo, metallic);
        half tDotH = dot(normalize(tangent + halfVector * _AnisoSpecularPosition), halfVector);
        half sinTH2 = 1.0 - tDotH * tDotH;
        half3 specular = pow(sinTH2, (smoothness + 0.05) * 250) * specColor * _AnisoSpecColor;
    ...
    half4 Fragment(VertexOutput input, half facing : VFACE) : SV_Target
    {
        ...
        for (uint lightIndex = 0u; lightIndex < pixelLightCount; ++lightIndex)
        {
            ...
            diff = lerp(addNDotL, nDotV, _SkinToggle);
    ...
    #if _SPECULAR_ON
        half3x3 tbnMat = half3x3(input.tangentWS.xyz, input.bitangentWS.xyz, input.normalWS.xyz);
        half3 halfVector = SafeNormalize(float3(viewDirWS) + float3(mainLight.direction));
        
        // 通过flowmap获取切线方向，并取高光遮罩
        half specMask;
        half3 flowTangent = GetTangentFlow(input.uvAndScreenUV.xy, normalWS, tbnMat, specMask);
        // 如果遮罩小于0.1，则计算正常高光，否则计算丝绸高光
        half3 silkSpec = SilkSpecular(flowTangent, halfVector, albedo.rgb, metallic, 1 - roughness, nDotL);
        half3 normalSpec = NormalSpeculr(albedo.rgb, metallic, max(0.01, metallic), roughness, normalWS, mainLight.direction, halfVector);
        half3 specColor = lerp(silkSpec, normalSpec, step(specMask, 0.1));
        specColor *= mainLight.color;
        specColor *= _SpecPower;

        for (uint lightIndex = 0u; lightIndex < pixelLightCount; ++lightIndex)
        {
            Light light = GetAdditionalLight(lightIndex, input.positionWS);
            halfVector = SafeNormalize(float3(viewDirWS) + float3(light.direction));
            half addNDotL = saturate(dot(normalWS, light.direction)) * 0.5 + 0.5;
            silkSpec = SilkSpecular(flowTangent, halfVector, albedo.rgb, metallic, 1 - roughness, addNDotL);
            normalSpec = NormalSpeculr(albedo.rgb, metallic, max(0.01, metallic), roughness, normalWS, light.direction, halfVector);
            specColor += lerp(silkSpec, normalSpec, step(specMask, 0.1)) * light.color * _SpecPower;
        }
