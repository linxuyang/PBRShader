Shader "MC/Character/Toon/BasicToonPBR"
{
    Properties
    {
        [Header(Basics)]
    ...
    half4 Fragment(VertexOutput input, half facing : VFACE) : SV_Target
    {
        ...
        half nDotL = saturate(dot(normalWS, mainLight.direction)) * mainLight.shadowAttenuation * 0.5 + 0.5;
        half nDotV = saturate(dot(normalWS, viewDirWS)) * mainLight.shadowAttenuation;
        ...
        specColor *= mainLight.color * mainLight.shadowAttenuation * _SpecPower;
        ...
        for (uint lightIndex = 0u; lightIndex < pixelLightCount; ++lightIndex)
        {
            ...
            halfVector = SafeNormalize(float3(light.direction) + float3(viewDirWS));
            specColor += NormalSpeculr(albedo.rgb, metallic, specMask, roughness, normalWS, light.direction,
                halfVector) * light.color * _SpecPower;
        }
        // 间接光
